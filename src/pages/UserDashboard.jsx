...
const UserDashboard = () => {
  ...
  const handleEdit = (todo) => {
    ...
    setSubtasks(todo.subtasks?.length ? todo.subtasks : [{ title: '', completed: false }]);
  };
    ...
    <div style={{ display: 'flex', height: '100vh' }}>
      ...
      <div className="main-content" style={{ flex: 1, padding: '20px', overflowY: 'auto' }}>
        <div style={{ marginBottom: '10px' }}>
          ...
          />
          <button onClick={clearFilter} disabled={!filterText}>Clear</button>
        </div>
        ...
        {activeTab === 'assigned' ? (
            ...
            {filteredTasks.length === 0 ? (
              ...
              <>
                  ...
                  <tbody>
                    {paginatedItems(filteredTasks).map((task) => (
                      <tr key={task._id}>
                        ...
                        <td>
                          <select value={task.status || 'assigned'} onChange={(e) => updateStatus(task._id, e.target.value)}>
                ...
                <div style={{ marginTop: '10px' }}>
                  ...
                  <button onClick={() => setCurrentPage((p) => Math.min(p + 1, totalPages(filteredTasks)))} disabled={currentPage === totalPages(filteredTasks)}>
                    Next
                  </button>
                </div>
              </>
          ...
          <>
            ...
            <form onSubmit={handleSubmit} style={{ marginBottom: '30px', padding: '10px', border: '1px solid #ddd', borderRadius: '8px', maxWidth: '600px' }}>
              <h3>{editTodo ? 'Edit To-do' : 'Add To-do'}</h3>

              <input type="text" placeholder="Title" required value={title} onChange={(e) => setTitle(e.target.value)} style={{ width: '100%', padding: '6px', marginBottom: '10px' }} />
              <textarea placeholder="Description" value={description} onChange={(e) => setDescription(e.target.value)} style={{ width: '100%', padding: '6px', marginBottom: '10px' }} />

              <label>
                Priority:{' '}
                <select value={priority} onChange={(e) => setPriority(e.target.value)} style={{ marginBottom: '10px' }}>
                  <option>Low</option>
              ...
              <label>
                Due Date:{' '}
                <input type="date" value={dueDate} onChange={(e) => setDueDate(e.target.value)} style={{ marginBottom: '10px' }} />
              </label>
              ...
              <div style={{ marginTop: '10px' }}>
                ...
                {subtasks.map((subtask, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', marginBottom: '6px' }}>
                    <input
                      ...
                      onChange={(e) => handleSubtaskChange(i, 'completed', e.target.checked)}
                    />
                    <input
                      type="text"
                ...
                <button type="button" onClick={addSubtaskField}>+ Add Subtask</button>
              </div>
            ...
            {filteredTodos.length === 0 ? (
              ...
              <>
                              ...
                              {todo.subtasks.map((st, idx) => (
                                <li key={idx}>
                                  <label>
                                    ...
                                    <button
                                      onClick={() => handleDeleteSubtask(todo._id, idx)}
                ...
                <div style={{ marginTop: '10px' }}>
                  ...
                  <button
                    onClick={() => setCurrentPage((p) => Math.min(p + 1, totalPages(filteredTodos)))}
